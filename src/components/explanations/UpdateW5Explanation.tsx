...
// Function to calculate tangent line at point x
const getTangentLine = (x: number, xValues: number[]) => {
  const slope = 2 * x; // derivative of x^2 is 2x
  const y = quadraticFunction(x);
  return xValues.map(xVal => (xVal - x) * slope + y);
};

const UpdateW5Explanation: React.FC = () => {
  const chartRef = useRef<any>(null); // Using any to avoid complex Chart.js types
  ...
  return (
    <div className="space-y-4">
      ...
      <ol className="list-decimal pl-6 space-y-3">
        <li>
          <span className="font-medium">Gradient Descent</span>: We move the weight in the opposite direction of the gradient 
          because the gradient points in the direction of steepest increase, and we want to minimize the error.
        </li>
        <li>
          <span className="font-medium">Learning Rate (<KatexComponent formula="\eta" />)</span>: This hyperparameter controls how big of a step we take in the 
          direction of the negative gradient. A higher learning rate means larger steps, which can lead to faster learning 
          but might cause overshooting. A lower learning rate is more stable but might require more iterations.
        </li>
        <li>
          <span className="font-medium">The Negative Sign</span>: Ensures we move against the gradient, towards the minimum of the error function.
        </li>
      </ol>
